    ...
    withEnv(['HOST=5.189.154.24',
        ...
        node('mypod') {
            ...
            stage('checkout & unit tests & build') {
                git url: 'https://github.com/khinkali/kafka-backup'
            ...
            stage('build image & git tag & docker push') {
                ...
                withCredentials([usernamePassword(credentialsId: 'github', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/khinkali/kafka-backup.git --tags"
                }

                container('docker') {
                    sh "docker build -t khinkali/kafka-backup:${env.VERSION} ."
                    withCredentials([usernamePassword(credentialsId: 'dockerhub', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
                        sh "docker login --username ${DOCKER_USERNAME} --password ${DOCKER_PASSWORD}"
                    }
                    sh "docker push khinkali/kafka-backup:${env.VERSION}"
                }
            }
            ...
            stage('deploy to prod') {
                try {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'github-api-token', passwordVariable: 'GITHUB_TOKEN', usernameVariable: 'GIT_USERNAME')]) {
                        container('curl') {
                            gitHubRelease(env.VERSION, 'khinkali', 'kafka-backup', GITHUB_TOKEN)
                        }
                    }
                    sh "sed -i -e 's/namespace: test/namespace: default/' kubeconfig.yml"
                    sh "sed -i -e 's/nodePort: 31082/nodePort: 30082/' kubeconfig.yml"
                    sh "sed -i -e 's/value: \"http:\\/\\/5.189.154.24:31190\\/auth\"/value: \"http:\\/\\/5.189.154.24:30190\\/auth\"/' kubeconfig.yml"
                    container('kubectl') {
                        sh "kubectl apply -f kubeconfig.yml"
                    }
                    container('curl') {
                        checkVersion(env.VERSION, 'http://5.189.154.24:30082/kafka-backup/resources/health', 1, 5)
                    }
                } catch (err) {
